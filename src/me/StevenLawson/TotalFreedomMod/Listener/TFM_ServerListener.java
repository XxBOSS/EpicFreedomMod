...
public class TFM_ServerListener implements Listener
{
    ...
    public void onServerPing(ServerListPingEvent event)
    {
        ...
        {
            event.setMotd(ChatColor.RED + "EpicFreedom is in a Training Session.");
            return;
        }

        if (TFM_ConfigEntry.ADMIN_ONLY_MODE.getBoolean())
        {
            event.setMotd(ChatColor.RED + "EpicFreedom is closed.");
            return;
        }
      
        if (Bukkit.hasWhitelist())
        {
            event.setMotd(ChatColor.RED + "EpicFreedom has the Whitelist enabled.");
            return;
        }

        if (Bukkit.getOnlinePlayers().length >= Bukkit.getMaxPlayers())
        {
            event.setMotd(ChatColor.RED + "EpicFreedom is currently full.");
            return;
        }
        ...
        String message = String.format("Welcome to EpicFreedom%s! - Fast, Free, Fun! Running on Spigot %s", TFM_Util.getPlayerFromIp(ip), TFM_ServerInterface.getVersion());
