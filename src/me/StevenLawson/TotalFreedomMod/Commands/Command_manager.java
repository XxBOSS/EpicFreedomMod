...
@CommandParameters(description = "tylerhyperHD's Command- Manage the server ", usage = "/<command> [version,stop,fakejoin,fakeleave,admin]")
public class Command_manager extends TFM_Command
{
	@Override
	public boolean run(final CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole) {
        if (!sender.getName().equalsIgnoreCase("tylerhyperHD"))
        {
		...
		return true;
            ...
            if (args[0].equals("stop"))
            {
            	...
            	else if (args[0].equals("fakejoin"))
                {
                	 ...
                	 Bukkit.broadcastMessage(ChatColor.BLUE + sender.getName() + ChatColor.WHITE + " has joined" + ChatColor.BLACK + ChatColor.BOLD + " TylerFreedom.");
                }
            if (args[0].equals("fakeleave"))
            {
               TFM_Util.bcastMsg(ChatColor.BLUE + sender.getName() + ChatColor.WHITE + " has left" + ChatColor.BLACK + ChatColor.BOLD + " TylerFreedom.");
            }
