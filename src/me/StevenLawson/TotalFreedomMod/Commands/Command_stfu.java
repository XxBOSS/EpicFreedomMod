...
public class Command_stfu extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        {
            TFM_Util.adminAction(sender.getName(), "Muting everyone on the server.", true);

            TFM_PlayerData playerdata;
            int counter = 0;
            for (Player player : server.getOnlinePlayers())
            {
                TFM_PlayerData.getPlayerData(player).setMuted(true);   
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    TFM_Util.adminAction(sender.getName(), "Muting " + player.getName(), true);
                    TFM_Util.adminAction("WARNING: " + sender.getName(), "has muted an administrator.", true);
                    playerdata.setMuted(true);
                    if (args.length == 2 && args[1].equalsIgnoreCase("-s"))
                    {
                        Command_smite.smite(player);
